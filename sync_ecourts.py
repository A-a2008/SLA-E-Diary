...
def main():
    ...
    while True:
        ...
        if args.update:
            logger.info(f"Update mode: Pending: {len(pending)} | Today: {len(refresh)} (queue totals — pending: {pending_total}, today: {refresh_total})")
        else:
        ...
        if refresh:
            log_label = 'TODAY' if args.update else 'REFRESH'
            _process_phase(log_label, refresh)
